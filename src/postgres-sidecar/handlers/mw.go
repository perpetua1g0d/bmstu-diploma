...
func metricsMiddleware(next http.HandlerFunc, serviceName string) http.HandlerFunc {
...
func authVeifyMiddleware(next http.HandlerFunc, serviceName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		start := time.Now()
		rw := &responseWriter{w, http.StatusOK, 0}
